...
import { RefreshCw, Search, Film, Eye, ImageIcon, AlertTriangle, Trash2 } from 'lucide-react';
...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  pending:                  { label: 'انتظار',           color: 'bg-gray-500/20 text-gray-400 border-gray-500/30' },
  generating:               { label: 'جاري التوليد',     color: 'bg-blue-500/20 text-blue-400 border-blue-500/30' },
  processing:               { label: 'معالجة',           color: 'bg-blue-500/20 text-blue-400 border-blue-500/30' },
  awaiting_script_approval: { label: 'موافقة السيناريو', color: 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30' },
  awaiting_image_approval:  { label: 'موافقة الصور',     color: 'bg-orange-500/20 text-orange-400 border-orange-500/30' },
  completed:                { label: 'مكتمل',            color: 'bg-green-500/20 text-green-400 border-green-500/30' },
  published:                { label: 'منشور',            color: 'bg-purple-500/20 text-purple-400 border-purple-500/30' },
  failed:                   { label: 'فشل',              color: 'bg-red-500/20 text-red-400 border-red-500/30' },
};

const STATUS_FILTER_OPTIONS = ['all','pending','processing','awaiting_script_approval','awaiting_image_approval','completed','published','failed'];
const STATUS_FILTER_LABELS: Record<string, string> = {
  ...
  completed: 'مكتمل', published: 'منشور', failed: 'فشل',
};
    ...
    <div className="space-y-5 pb-12">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">{total} حلقة إجمالاً</p>
        </div>
          ...
          <div className="divide-y divide-white/5">
            {episodes.map(ep => {
              ...
              return (
                <div
                  key={ep.id}
                  className={`flex items-center gap-3 px-5 py-3.5 hover:bg-white/5 transition-all ${needsReview ? 'bg-yellow-500/5' : ''}`}
                >
                  {/* Thumbnail */}
